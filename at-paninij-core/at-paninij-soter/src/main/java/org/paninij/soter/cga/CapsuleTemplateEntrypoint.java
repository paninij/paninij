...
public class CapsuleTemplateEntrypoint extends DefaultEntrypoint
{
    ...
    protected int makeReceiver(AbstractRootMethod root)
    {
        ...
        Stream.concat(getChildCapsuleDecls(template),
              ...
              .forEach(f -> addMockup(root, f, receiverValueNumber));
     ...
     */
    protected void addMockup(AbstractRootMethod root, IField field, int receiverValueNumber)
    {
        TypeReference capsuleInterface = field.getFieldTypeReference();

        TypeReference dummyType = getCapsuleMockupClassReference(capsuleInterface);
        if (dummyType == null)
        {
