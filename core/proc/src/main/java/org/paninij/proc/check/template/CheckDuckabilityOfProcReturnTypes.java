 ...
 */
public class CheckDuckabilityOfProcReturnTypes implements TemplateCheck
{
    private final DuckabilityChecker checker;

    public CheckDuckabilityOfProcReturnTypes(ProcessingEnvironment procEnv) {
        this.checker = new DuckabilityChecker(procEnv);
    }
        ...
        for (Element elem : template.getEnclosedElements()) {
            if (isDuckProcedure(elem)) {
                ...
                if (! result.ok()) {
                    ...
                    return error(err, CheckDuckabilityOfProcReturnTypes.class, elem);
                }
            }
        }
        return OK;
